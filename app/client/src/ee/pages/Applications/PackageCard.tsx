...
function PackageCard({ isFetchingPackages, isMobile, pkg }: PackageCardProps) {
  ...
  const isDeleteClickedRef = useRef(false);
  ...
  const closeMenu = useCallback(() => {
    setIsMenuOpen(false);
    setIsDeleting(false);
  }, [setIsMenuOpen, setIsDeleting]);

  const handleMenuOnClose = useCallback(
    (open: boolean) => {
      if (!open) {
        if (isDeleteClickedRef.current) {
          isDeleteClickedRef.current = false;
        } else {
          closeMenu();
  ...
  }, [setShowOverlay, dispatch, deletePackage, packageId]);
  ...
  const moreActionItems = useMemo(() => {
    ...
    if (hasDeletePermission) {
      if (isDeleting) {
        ...
        options.push({
          onSelect: () => {
            isDeleteClickedRef.current = true;
            setIsDeleting(true);
            setIsMenuOpen(true);
          },
          children: "Delete",
