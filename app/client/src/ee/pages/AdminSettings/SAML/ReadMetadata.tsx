...
export const MENU_ITEMS_MAP: MenuItemsProps[] = [
  {
    ...
    inputs: [
      ...
      {
        className: "t--sso-saml-provider-name",
        subText: "* Name of the provider which shows up on the login button",
        label: "Provider name",
...
function MetadataForm(
  ...
  const { activeTabIndex = 0, defaultSettings, settings } = props;
  ...
  const submit = () => {
    Object.keys(defaultSettings).forEach((setting) => {
      if (
        ...
        defaultSettings[setting] === props.settingsConfig[setting]
      ) {
...
const MetadataReduxForm = connect(
  (
    ...
    const defaultSettings = {
      metadataEntityId: `${window.location.origin}${BASE_URL}auth/realms/appsmith`,
    };
    const newProps: any = {
      settings: {},
      settingsConfig: {
        claims: [{ key: "", value: "" }],
        metadataEntityId:
          settingsConfig.metadataEntityId || defaultSettings.metadataEntityId,
        ...settingsConfig,
      },
      defaultSettings,
